        ...
        try {
            ...
            for (const c of calcs) {
                ...
                if (prevStatus === 'pending' && status === 'completed') {
                    clearPendingCalculationState();
                    toast.success(
                        t('notifications.pensionCalculationSuccessToast'),
                    );
                    hasStatusChanges = true;
                } else if (prevStatus === 'pending' && status === 'failed') {
                    clearPendingCalculationState();
                    toast.error(
                        c.error_message || t('pensionCalc.failedHistoryDesc'),
                    );
                    hasStatusChanges = true;
                } else if (
                    isCalculationsBootstrapped &&
                    !knownCalcStatuses.has(c.id)
                ) {
                    hasStatusChanges = true;
                }
        ...
        try {
            ...
            if (!isCalcBootstrapped) {
                for (const n of items) {
                    const isCalcType =
                        n.type === 'calculation' || n.type === 'calc_completed';
            ...
            for (const n of items) {
                ...
                if (isCalcType && !n.is_seen && !notifiedCalcIds.has(n.id)) {
                    ...
                    n.type === 'error' &&
                    !n.is_seen &&
                    !notifiedCalcIds.has(n.id)
                ) {
                    notifiedCalcIds.add(n.id);
                    saveNotifiedCalcSet(notifiedCalcIds);
                    const msg =
                        (n.translations &&
                            (n.translations.en || n.translations.uk)) ||
            ...
            for (const d of docs) {
                ...
                if (['completed', 'processed', 'success'].includes(status)) {
                    if (
                        ...
                        saveNotifiedSet(notified);
                        toast.success(
                            t('notifications.ocrSuccessToast').replace(
                                ':file',
                                filename,
                            ),
                        );
                    ...
                    if (
                        ...
                        saveNotifiedSet(notified);
                        toast.error(
                            t('notifications.ocrFailedToast').replace(
                                ':file',
                                filename,
                            ),
                        );
                ...
                if (echo) {
                    const handleCalcEvent = async () => {
                        ...
                        window.dispatchEvent(
                            new CustomEvent('calculations-updated'),
                        );
                        window.dispatchEvent(
