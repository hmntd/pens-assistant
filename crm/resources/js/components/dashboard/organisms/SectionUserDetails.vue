...
import type { User as UserType, UserDetailsForm } from '@/types';

const { t } = useI18n();
const page = usePage();
const user = page.props.auth?.user as UserType | undefined;
...
const form = useForm<UserDetailsForm>({
    ...
    pension_type: 'age',
    target_retirement_year: user?.target_retirement_year ?? null,
    benefits: Array.isArray(user?.benefits) ? user.benefits : [],
});
...
function saveDetails() {
    if (form.target_retirement_year === ('' as any)) {
        form.target_retirement_year = null;
    }
...
<template>
    <div class="space-y-8">
        ...
        >
            ...
            <div
                v-if="savedSuccess"
                class="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/20 px-3 py-1 text-xs font-semibold text-emerald-600 dark:text-emerald-400"
            >
        ...
        >
            ...
            <div
                class="h-fit space-y-6 self-start rounded-2xl border border-slate-200/80 bg-white/70 p-6 shadow-sm backdrop-blur-md lg:col-span-6 dark:border-zinc-800/80 dark:bg-zinc-950/80"
            >
                <h3
                    class="flex items-center gap-2 border-b border-slate-100 pb-3 text-base font-bold text-slate-900 dark:border-zinc-800/60 dark:text-white"
                >
                ...
                <div class="grid gap-2">
                    ...
                    >
                        ...
                        <span
                            v-if="!form.gender"
                            class="text-[10px] font-bold text-amber-500"
                            >{{ t('gender.notSpecified') }}</span
                        >
                    </Label>
                    <GenderSelectButton id="gender" v-model="form.gender" />
                </div>

                <!-- Date of Birth Picker -->
                <div class="grid gap-2">
                    <Label for="date_of_birth">{{
                        t('profileDetails.dateOfBirth')
                    }}</Label>
                    <DatePicker
                        id="date_of_birth"
                ...
                <div class="grid gap-2">
                    ...
                    >
                        ...
                        <span
                            v-if="!form.target_retirement_year"
                            class="text-[10px] font-bold text-amber-500"
                            >{{ t('profileDetails.notSpecifiedWarning') }}</span
                        >
                    </Label>
                    <YearPicker
                        id="target_year"
                        ...
                        :max-year="2070"
                        :placeholder="
                            t('profileDetails.selectTargetYearPlaceholder')
                        "
                    />
                </div>
            </div>
            ...
            >
                ...
                <div class="grid gap-2">
                    <Label for="disability_group" class="font-bold">{{
                        t('profileDetails.disabilityGroupLabel')
                    }}</Label>
                ...
                <div class="space-y-3 pt-2">
                    <Label class="font-bold">{{
                        t('profileDetails.benefitStatusLabel')
                    }}</Label>
                    ...
                    <div
                        ...
                        :class="
                            form.benefits?.includes('chornobyl')
                                ? 'border-main/50 bg-main/10 dark:bg-main/15'
                                : 'border-slate-200 bg-slate-50/50 dark:border-zinc-800 dark:bg-zinc-900/50'
                        "
                    >
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                >
                                    {{
                                        t(
                                            'profileDetails.benefitChornobylTitle',
                                        )
                                    }}
                                </h4>
                                <p
                                ...
                                >
                                    {{
                                        t('profileDetails.benefitChornobylDesc')
                                    }}
                                </p>
                            </div>
                        </div>
                        <input
                            type="checkbox"
                            :checked="form.benefits?.includes('chornobyl')"
                    ...
                    <div
                        ...
                        class="flex cursor-pointer items-center justify-between rounded-xl border p-3 transition-colors"
                        :class="
                            form.benefits?.includes('combat_participant')
                                ? 'border-main/50 bg-main/10 dark:bg-main/15'
                                : 'border-slate-200 bg-slate-50/50 dark:border-zinc-800 dark:bg-zinc-900/50'
                        "
                    >
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h4
                                    class="text-xs font-bold text-slate-900 dark:text-white"
                                >
                                    {{ t('profileDetails.benefitUbdTitle') }}
                                </h4>
                                <p
                                    class="text-[10px] text-slate-500 dark:text-zinc-400"
                                >
                                    {{ t('profileDetails.benefitUbdDesc') }}
                                </p>
                            </div>
                        </div>
                        <input
                ...
                <div class="pt-4">
                    <Button
                        type="submit"
                        class="w-full cursor-pointer bg-main font-bold text-slate-950 shadow-md transition-all duration-200 hover:scale-[1.005] hover:bg-main-dark active:scale-[0.985]"
                        :disabled="form.processing"
                    >
                        <Save class="mr-2 h-4 w-4" />
                        {{
                            form.processing
                                ? t('profileDetails.savingBtn')
                                : t('profileDetails.saveBtn')
                        }}
                    </Button>
                </div>
            </div>
