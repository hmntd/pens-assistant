...
<template>
    <div class="space-y-8">
        ...
        <form @submit.prevent="saveDetails" class="grid grid-cols-1 gap-8 lg:grid-cols-12 items-start">
            ...
            <div
                class="lg:col-span-6 rounded-2xl border border-slate-200/80 bg-white/70 p-6 shadow-sm backdrop-blur-md dark:border-zinc-800/80 dark:bg-zinc-950/80 space-y-6 self-start h-fit">
                <h3
                    class="text-base font-bold text-slate-900 dark:text-white border-b border-slate-100 dark:border-zinc-800/60 pb-3 flex items-center gap-2">
                ...
                <div class="grid gap-2">
                    <Label for="gender" class="flex items-center justify-between">
                        ...
                        <span v-if="!form.gender" class="text-[10px] text-amber-500 font-bold">{{
                ...
                <div class="grid gap-2">
                    ...
                    <DatePicker id="date_of_birth" v-model="form.date_of_birth" :disable-future="true"
                ...
                <div class="grid gap-2">
                    <Label for="target_year" class="flex items-center justify-between">
                        ...
                        <span v-if="!form.target_retirement_year" class="text-[10px] text-amber-500 font-bold">{{
                            t('profileDetails.notSpecifiedWarning') }}</span>
                    </Label>
                    <YearPicker id="target_year" v-model="form.target_retirement_year" :min-year="1990" :max-year="2070"
            ...
            <div
                ...
                <div class="grid gap-2">
                    <Label for="disability_group" class="font-bold">{{ t('profileDetails.disabilityGroupLabel')
                    }}</Label>
                    <DisabilityGroupSelect id="disability_group" v-model="form.disability_group" />
                </div>
                ...
                <div class="space-y-3 pt-2">
                    ...
                    <div @click="toggleBenefit('chornobyl')"
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h4 class="text-xs font-bold text-slate-900 dark:text-white">{{
                                    t('profileDetails.benefitChornobylTitle') }}</h4>
                                <p class="text-[10px] text-slate-500 dark:text-zinc-400">{{
                                    t('profileDetails.benefitChornobylDesc') }}</p>
                            </div>
                        </div>
                        <input type="checkbox" :checked="form.benefits?.includes('chornobyl')"
                            class="h-4 w-4 rounded accent-main" />
                    </div>

                    <div @click="toggleBenefit('combat_participant')"
                        class="flex items-center justify-between p-3 rounded-xl border transition-colors cursor-pointer"
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h4 class="text-xs font-bold text-slate-900 dark:text-white">{{
                                    t('profileDetails.benefitUbdTitle') }}</h4>
                                <p class="text-[10px] text-slate-500 dark:text-zinc-400">{{
                                    t('profileDetails.benefitUbdDesc') }}</p>
                            </div>
                        </div>
                        <input type="checkbox" :checked="form.benefits?.includes('combat_participant')"
                            class="h-4 w-4 rounded accent-main" />
                    </div>
                </div>
