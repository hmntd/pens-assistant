    ...
    <div>
            ...
            <div class="grid grid-cols-1 gap-8 lg:grid-cols-12 items-start">
                ...
                <div class="lg:col-span-7 space-y-6">
                    <div
                        class="rounded-2xl border border-slate-200/80 bg-white/70 p-6 dark:border-zinc-800/80 dark:bg-zinc-950/80 space-y-4">
                        <div
                            class="flex justify-between items-center border-b border-slate-100 dark:border-zinc-800/60 pb-3">
        ...
        <div v-else class="space-y-8">
            ...
            <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                ...
                <span
                    class="inline-flex items-center gap-1.5 rounded-full bg-main/15 px-3 py-1 text-xs font-bold text-main-dark dark:text-main shrink-0">
                    <Calendar class="h-3.5 w-3.5" />
                    {{ t('documents.totalService') }} {{ totalWorkedYearsCount }} {{ t('documents.yrs') }} ({{
                        totalAccumulatedMonthsCount }} {{ t('documents.months') }})
                </span>
            </div>
            ...
            <div class="grid grid-cols-1 gap-8 lg:grid-cols-12 items-start">
                ...
                <div class="lg:col-span-5 space-y-6 self-start h-fit">
                    ...
                    <div
                        ...
                        <label
                            ...
                            <div v-if="uploadForm.processing"
                                class="absolute bottom-0 left-0 right-0 h-1.5 bg-slate-200 dark:bg-zinc-800">
                                <div class="h-full bg-main transition-all duration-300 ease-out"
                                    :style="{ width: `${uploadForm.progress?.percentage ?? 0}%` }"></div>
                            </div>
                        ...
                        <div v-if="uploadForm.processing" class="space-y-1.5 pt-1">
                            ...
                            <div
                                class="w-full bg-slate-200 dark:bg-zinc-800 h-2.5 rounded-full overflow-hidden p-0.5 border border-slate-300/50 dark:border-zinc-700/50">
                                <div class="bg-gradient-to-r from-main-dark to-main h-full rounded-full transition-all duration-300 shadow-sm"
                                    :style="{ width: `${uploadForm.progress?.percentage ?? 0}%` }"></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="relative flex items-center justify-center py-2">
                        <div class="absolute inset-0 flex items-center">
                            <div class="w-full border-t-2 border-dashed border-slate-300/80 dark:border-zinc-800/80">
                            </div>
                        </div>
                        <div
                            class="relative bg-slate-100 dark:bg-zinc-900 px-4 py-1 text-xs font-black uppercase tracking-wider text-slate-500 dark:text-zinc-400 rounded-full border border-slate-200 dark:border-zinc-800 shadow-sm">
                    ...
                    <div
                        ...
                        <h3
                            ...
                            <div
                                class="flex items-center gap-1 bg-slate-100 dark:bg-zinc-900 p-1 rounded-xl text-[10px] font-bold">
                                <button type="button" @click="isRangeMode = false"
                                ...
                                <button type="button" @click="isRangeMode = true"
                                    class="px-2 py-1 rounded-lg transition-colors cursor-pointer"
                                    :class="isRangeMode ? 'bg-main text-slate-950' : 'text-slate-500 dark:text-zinc-400'">
                        ...
                        <form @submit.prevent="submitManualTaxHistory" class="space-y-4">
                            ...
                            <template v-if="isRangeMode">
                                <div class="grid grid-cols-2 gap-3">
                                    <div class="grid gap-1.5">
                                        ...
                                        <YearPicker id="from_year" v-model="manualTaxForm.from_year" :min-year="1950"
                                            :max-year="currentYear - 1" />
                                    </div>
                                    <div class="grid gap-1.5">
                                        <Label for="to_year" class="text-xs">{{ t('documents.toYear') }}</Label>
                                        <YearPicker id="to_year" v-model="manualTaxForm.to_year"
                                            :min-year="manualTaxForm.from_year ? manualTaxForm.from_year + 1 : 1951"
                            ...
                            <template v-else>
                                <div class="grid gap-1.5">
                                    ...
                                    <YearPicker id="single_year" v-model="manualTaxForm.year" :min-year="1950"
                                        :max-year="currentYear" />
                                </div>
                            </template>

                            <!-- Pre-2000 Salary Pass Switch/Toggle -->
                            <div v-if="isPre2000Selected"
                                class="rounded-xl border border-amber-200/80 bg-amber-50/60 dark:border-amber-900/40 dark:bg-amber-950/30 p-3.5 space-y-2">
                                <div class="flex items-center justify-between gap-3">
                                    <Label for="pass_salary_pre_2000"
                                        class="text-xs font-bold text-slate-800 dark:text-zinc-200 cursor-pointer">
                                        {{ t('documents.includePre2000SalaryLabel') }}
                                    </Label>
                                    <button type="button" id="pass_salary_pre_2000"
                                        ...
                                        <span
                                            ...
                                            :class="passSalaryPre2000 ? 'translate-x-5' : 'translate-x-0'"></span>
                                    </button>
                                </div>
                                <p class="text-[11px] text-amber-800 dark:text-amber-300/90 leading-relaxed">
                                    {{ passSalaryPre2000 ? t('documents.pre2000SalaryNoticeWith') :
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <div v-if="!isPre2000Selected || passSalaryPre2000" class="grid gap-1.5">
                                    <Label for="monthly_salary" class="text-xs">{{ t('documents.monthlySalary')
                                        }}</Label>
                                    <Input id="monthly_salary" type="number" step="100" min="0"
                                        v-model="manualTaxForm.monthly_salary" required />
                                </div>
                                <div
                                    :class="(!isPre2000Selected || passSalaryPre2000) ? 'grid gap-1.5' : 'col-span-2 grid gap-1.5'">
                                    <Label for="months_worked" class="text-xs">{{ t('documents.monthsInYear') }}</Label>
                                    <Input id="months_worked" type="number" min="1" max="12"
                                        v-model="manualTaxForm.months_worked" required />
                                </div>
                            </div>
                ...
                <div class="lg:col-span-7 space-y-6">
                    ...
                    <div
                        class="rounded-2xl border border-slate-200/80 bg-white/70 p-6 shadow-sm backdrop-blur-md dark:border-zinc-800/80 dark:bg-zinc-950/80 space-y-4">
                        <h3
                            class="text-base font-bold text-slate-900 dark:text-white flex items-center justify-between border-b border-slate-100 dark:border-zinc-800/60 pb-3">
                            <span class="flex items-center gap-2">
                        ...
                        <template v-if="taxHistoriesList.length > 0">
                            <div
                                class="overflow-x-auto max-h-64 overflow-y-auto [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-slate-300 dark:[&::-webkit-scrollbar-thumb]:bg-zinc-800 [&::-webkit-scrollbar-thumb]:rounded-full">
                                <table class="w-full text-left text-xs">
                                    <thead>
                                        <tr
                                            class="border-b border-slate-200 dark:border-zinc-800 text-slate-400 font-semibold">
                                    ...
                                    <tbody class="divide-y divide-slate-100 dark:divide-zinc-900">
                                        <tr v-for="item in taxHistoriesList" :key="item.id"
                                            class="hover:bg-slate-50/50 dark:hover:bg-zinc-900/50">
                                            <td class="py-2.5 font-bold text-slate-900 dark:text-white">{{ item.year }}
                                            </td>
                                            <td class="py-2.5 text-right font-mono text-slate-700 dark:text-zinc-300">
                                                <template v-if="item.annual_income > 0">
                                                    {{ Number(item.annual_income / (item.months_worked ||
                                                        12)).toLocaleString('uk-UA', { minimumFractionDigits: 0 }) }} ₴
                                                </template>
                                                <template v-else>
                                                    <span
                                                        class="text-[10px] italic text-amber-700 dark:text-amber-400 font-semibold bg-amber-50 dark:bg-amber-950/40 px-2 py-0.5 rounded-md border border-amber-200/60 dark:border-amber-900/40">
                                            ...
                                            <td class="py-2.5 text-right">
                                                <div class="flex items-center justify-end gap-1">
                                                    <button @click="openEditTaxModal(item)" type="button"
                                                        class="text-slate-400 hover:text-main cursor-pointer p-1"
                                                        :title="t('documents.editMonthlySalary')">
                                                        <Pencil class="h-3.5 w-3.5" />
                                                    </button>
                                                    <button @click="confirmDeleteTaxHistory(item)" type="button"
                    ...
                    <div
                        ...
                        <h3
                            ...
                            <span class="text-xs text-slate-400 font-normal">{{ t('documents.filesCount') }} {{
                                ...
                                <div v-for="doc in documentsList" :key="doc.id"
                                        ...
                                        <div class="min-w-0">
                                            ...
                                            <div class="flex items-center gap-2 mt-0.5">
                                                <span
                                                    ...
                                                    {{ isDocCompleted(doc) ? t('documents.processed') : isDocFailed(doc)
                                                        ? t('documents.failed') : t('documents.pending') }}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex items-center gap-1 shrink-0">
                                        <Button size="icon" variant="ghost"
                                            ...
                                            @click="openReviewModal(doc)" title="Review Document">
            ...
            <div v-if="isReviewModalOpen && selectedDoc"
                class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/75 p-4 backdrop-blur-sm">
                <div
                    class="relative w-full max-w-4xl max-h-[90vh] flex flex-col rounded-3xl border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shadow-2xl overflow-hidden">
                    <!-- Modal Header -->
                    <div
                        class="flex items-center justify-between border-b border-slate-100 dark:border-zinc-900 px-6 py-4 bg-slate-50/50 dark:bg-zinc-900/50">
                            ...
                            <div>
                                <h3
                                    ...
                                    <span
                                        ...
                                        {{ isDocCompleted(selectedDoc) ? t('documents.processed') :
                                            isDocFailed(selectedDoc) ?
                                                t('documents.failed') : t('documents.pending') }}
                                    </span>
                                </h3>
                        ...
                        <div class="flex items-center gap-2">
                            <a :href="getFileStreamUrl(selectedDoc.id)" target="_blank" download
                    ...
                    <div class="flex-1 overflow-y-auto p-6 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div
                                class="rounded-2xl border border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-900 p-2 flex flex-col items-center justify-center min-h-[350px]">
                                <template v-if="isCheckingFile">
                                    <div
                                        class="flex flex-col items-center justify-center p-8 text-center space-y-2 text-slate-400">
                                ...
                                <template v-else-if="isImageError">
                                    <div class="flex flex-col items-center justify-center p-8 text-center space-y-3">
                                        <div
                                            class="h-14 w-14 rounded-2xl bg-amber-50 dark:bg-amber-950/40 text-amber-500 flex items-center justify-center border border-amber-200/60 dark:border-amber-900/40 shadow-xs">
                                            <FileX class="h-7 w-7" />
                                        </div>
                                        <div>
                                            ...
                                            <p
                                                class="text-xs text-slate-500 dark:text-zinc-400 mt-1 max-w-xs leading-relaxed">
                                ...
                                <template v-else-if="isPdfFile(selectedDoc.original_filename)">
                                    <iframe :src="getFileStreamUrl(selectedDoc.id)"
                                        class="w-full h-[400px] rounded-xl border-0"
                                        @error="isImageError = true"></iframe>
                                </template>
                                <template v-else>
                                    <img :src="getFileStreamUrl(selectedDoc.id)" :alt="selectedDoc.original_filename"
                            ...
                            <div class="space-y-4 flex flex-col justify-between">
                                <div class="space-y-4">
                                    <h4
                                        class="text-xs font-extrabold uppercase tracking-wider text-slate-400 border-b border-slate-100 dark:border-zinc-800 pb-2">
                                        {{ t('documents.ocrResultTitle') }}
                                    </h4>

                                    <div v-if="getRawText(selectedDoc)"
                                        class="rounded-xl bg-slate-900 p-3 text-[11px] font-mono text-emerald-400 max-h-48 overflow-y-auto border border-zinc-800 whitespace-pre-wrap">
                                        {{ getRawText(selectedDoc) }}
                                    </div>
                                    <div v-else
                                        class="p-4 text-center text-xs text-slate-400 rounded-xl bg-slate-50 dark:bg-zinc-900">
                                        {{ isDocPending(selectedDoc) ? t('documents.ocrProcessing') :
                                    ...
                                    <div v-if="getExtractedData(selectedDoc)" class="space-y-2">
                                        ...
                                        <pre
                                            class="rounded-xl bg-slate-100 dark:bg-zinc-900 p-3 text-[10px] font-mono text-slate-700 dark:text-zinc-300 max-h-36 overflow-y-auto border border-slate-200 dark:border-zinc-800">
                                {{ JSON.stringify(getExtractedData(selectedDoc), null, 2) }}</pre>
                                    </div>
                                </div>

                                <div class="pt-4 border-t border-slate-100 dark:border-zinc-900 flex justify-end gap-2">
                                    <Button variant="outline" size="sm" @click="closeReviewModal"
        ...
        <Dialog :open="isEditTaxModalOpen" @update:open="isEditTaxModalOpen = $event">
            <DialogContent class="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
                ...
                <form @submit.prevent="submitEditTaxHistory" class="space-y-6 pt-4">
                    ...
                    <div v-if="editingTaxHistory && editingTaxHistory.year < 2000"
                        class="rounded-xl border border-amber-200/80 bg-amber-50/60 dark:border-amber-900/40 dark:bg-amber-950/30 p-3.5 space-y-2">
                        <div class="flex items-center justify-between gap-3">
                            <Label for="edit_pass_salary_toggle"
                                class="text-xs font-bold text-slate-800 dark:text-zinc-200 cursor-pointer">
                                {{ t('documents.includePre2000SalaryLabel') }}
                            </Label>
                            <button type="button" id="edit_pass_salary_toggle"
                                @click="editPassSalaryPre2000 = !editPassSalaryPre2000"
                                class="relative inline-flex h-5 w-10 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none"
                                :class="editPassSalaryPre2000 ? 'bg-main' : 'bg-slate-300 dark:bg-zinc-700'">
                                <span
                                    class="pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow-sm ring-0 transition duration-200 ease-in-out"
                                    :class="editPassSalaryPre2000 ? 'translate-x-5' : 'translate-x-0'"></span>
                            </button>
                        </div>
                        <p class="text-[11px] text-amber-800 dark:text-amber-300/90 leading-relaxed">
                            {{ editPassSalaryPre2000 ? t('documents.pre2000SalaryNoticeWith') :
                                t('documents.pre2000SalaryNoticeWithout') }}
                        </p>
                    </div>

                    <!-- 12 Months Grid -->
                    <div v-if="!editingTaxHistory || editingTaxHistory.year >= 2000 || editPassSalaryPre2000"
                        class="grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-[50vh] overflow-y-auto pr-2">
                        <div v-for="m in 12" :key="m"
                            class="flex items-center justify-between gap-3 p-2.5 rounded-xl border border-slate-200/80 dark:border-zinc-800/80 bg-slate-50/50 dark:bg-zinc-900/50">
                            <span
                                class="text-xs font-semibold text-slate-700 dark:text-zinc-300 w-28 shrink-0 flex items-center gap-1.5">
                                <span
                                    class="w-5 h-5 rounded-full bg-main/20 text-main font-bold text-[10px] flex items-center justify-center">{{
                                        m }}</span>
                                {{ monthNames[m - 1] }}
                            </span>
                            <div class="relative flex-1">
                                <Input v-model.number="monthlyBreakdownForm[m]" type="number" step="0.01" min="0"
                                    class="text-right text-xs font-mono pr-7 h-9" placeholder="0.00" />
                                <span
                                    class="absolute right-2.5 top-2.5 text-xs text-slate-400 font-bold pointer-events-none">₴</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div
                        class="flex items-center justify-between p-4 rounded-xl bg-main/10 border border-main/30 dark:bg-main/15">
                        <span class="text-xs font-bold text-slate-900 dark:text-white">
                    ...
                    <!-- Action Buttons -->
                    <DialogFooter
                        class="flex items-center justify-end gap-3 pt-2 border-t border-slate-100 dark:border-zinc-800">
                        <Button type="button" variant="outline" @click="isEditTaxModalOpen = false"
                            :disabled="isSubmittingEditTax" class="h-10 text-xs font-bold cursor-pointer">
                            {{ t('documents.cancelBtn') }}
                        </Button>

                        <Button type="submit" :disabled="isSubmittingEditTax"
            ...
            <DialogContent class="sm:max-w-md">
                <DialogHeader>
                    <DialogTitle
                        ...
                        <div
                            class="h-9 w-9 rounded-xl bg-red-500/10 text-red-600 dark:text-red-400 flex items-center justify-center border border-red-500/20 shrink-0">
                            <Trash2 class="h-4 w-4" />
                        </div>
                        {{ deleteTarget?.type === 'document' ? t('documents.deleteConfirmDocTitle') :
                            t('documents.deleteConfirmTaxTitle') }}
                    </DialogTitle>
                    <DialogDescription class="text-xs text-slate-500 dark:text-zinc-400 leading-relaxed pt-2">
                        {{ deleteTarget?.type === 'document' ? t('documents.deleteConfirmDocMsg') :
                            t('documents.deleteConfirmTaxMsg') }}
                        <span v-if="deleteTarget?.title"
                            class="block font-bold text-slate-900 dark:text-white mt-1.5 p-2 rounded-xl bg-slate-100 dark:bg-zinc-900 border border-slate-200/60 dark:border-zinc-800">
                            {{ deleteTarget.title }}
                        </span>
                    </DialogDescription>
                </DialogHeader>
                ...
                <DialogFooter
                    class="flex items-center justify-end gap-2.5 pt-4 border-t border-slate-100 dark:border-zinc-800">
                    <Button type="button" variant="outline" @click="isConfirmDeleteModalOpen = false"
                        :disabled="isDeletingRecord" class="h-9 text-xs font-bold cursor-pointer">
                        {{ t('documents.cancelBtn') }}
                    </Button>
                    <Button type="button" @click="executeDeleteRecord" :disabled="isDeletingRecord"
