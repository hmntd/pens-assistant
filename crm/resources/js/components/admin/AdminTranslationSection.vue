...
async function saveTranslationRow(item: TranslationItem) {
    ...
    try {
        ...
        const data = await res.json();
        if (res.ok && data.status === 'success') {
            toast.success(data.message || t('adminTranslations.successSave').replace(':key', item.key));
        } else {
    ...
    <div class="space-y-6">
        ...
        <div
            ...
            <div class="relative flex-1 max-w-md">
                ...
                <input v-model="searchQuery" type="text" :placeholder="t('adminTranslations.searchPlaceholder')"
        ...
        <div
                ...
                <table class="w-full text-left text-xs sm:text-sm min-w-[700px]">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-zinc-800/80 font-medium">
                        ...
                        <tr v-for="item in translations" :key="item.key"
                            ...
                            <td class="p-3.5">
                                <textarea v-model="item.uk" rows="2"
                                    class="w-full p-2 text-xs bg-slate-50 dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-xl focus:outline-none focus:ring-1 focus:ring-main"></textarea>
                            </td>
                            <td class="p-3.5">
                                <textarea v-model="item.en" rows="2"
                                    class="w-full p-2 text-xs bg-slate-50 dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-xl focus:outline-none focus:ring-1 focus:ring-main"></textarea>
                            </td>
                            <td class="p-3.5 text-right">
                                <button @click="saveTranslationRow(item)" :disabled="item.is_saving"
                                    ...
                                    <span>{{ item.is_saving ? t('adminTranslations.btnSaving') :
                                        t('adminTranslations.btnSave') }}</span>
                                </button>
                            </td>
                        </tr>
                    </tbody>
            ...
            <div
                ...
                <div>{{ t('adminTranslations.totalKeys') }} <span class="font-bold text-slate-900 dark:text-white">{{
                    totalRecords }}</span></div>
                <div class="flex items-center gap-2">
                    <button :disabled="currentPage <= 1" @click="currentPage--; fetchTranslations();"
                    ...
                    <button :disabled="currentPage >= lastPage" @click="currentPage++; fetchTranslations();"
                        class="px-3 py-1.5 rounded-lg border border-slate-200 dark:border-zinc-800 disabled:opacity-40 cursor-pointer">
        ...
        <div v-if="showAddModal"
            ...
            <div
                class="bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-3xl w-full max-w-lg p-6 shadow-2xl space-y-4 relative">
                ...
                <div class="flex items-center gap-2 text-main-dark dark:text-main">
                    ...
                    <h3 class="text-base font-extrabold text-slate-900 dark:text-white">{{
                        t('adminTranslations.modalTitle') }}</h3>
                </div>

                <div class="space-y-3 text-xs">
                    <div>
                        <label class="block text-slate-500 font-bold mb-1">{{ t('adminTranslations.labelKey') }}</label>
                        <input v-model="newKey" type="text" placeholder="admin.newKey"
                    ...
                    <div>
                        ...
                        <textarea v-model="newUkVal" rows="3" :placeholder="t('adminTranslations.placeholderUk')"
                            class="w-full p-2.5 bg-slate-50 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-main"></textarea>
                    </div>
                    <div>
                        <label class="block text-slate-500 font-bold mb-1">{{ t('adminTranslations.labelEn') }}</label>
                        <textarea v-model="newEnVal" rows="3" :placeholder="t('adminTranslations.placeholderEn')"
                            class="w-full p-2.5 bg-slate-50 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-main"></textarea>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end gap-2 pt-2">
                    ...
                    <button @click="handleAddKey()" :disabled="isSubmittingNewKey" type="button"
                        ...
                        <span>{{ isSubmittingNewKey ? t('adminTranslations.btnSaving') : t('adminTranslations.btnAdd')
                        }}</span>
                    </button>
                </div>
            </div>
