    ...
    <div class="space-y-6">
        ...
        <div
            ...
            <div class="relative flex-1 max-w-md">
                ...
                <input v-model="searchQuery" type="text" :placeholder="t('adminDocuments.searchPlaceholder')"
        ...
        <div
                            ...
                            <td class="p-3.5">
                                <div class="flex items-center gap-2">
                                    ...
                                    <div>
                                        ...
                                        <span
                                            class="text-[10px] font-extrabold uppercase px-1.5 py-0.5 rounded bg-slate-100 dark:bg-zinc-800 text-slate-500">
                                            {{ doc.document_type }}
                                        </span>
                                    </div>
                                </div>
                            </td>
                            <td class="p-3.5 font-mono text-slate-600 dark:text-zinc-300">{{ doc.formatted_file_size }}
                            </td>
                            <td class="p-3.5 text-slate-500 font-mono text-xs">{{ doc.created_at }}</td>
                            <td class="p-3.5 text-right">
                                <div class="flex items-center justify-end gap-1.5">
                                    <button @click="previewDocument(doc)" :title="t('adminUsers.titleViewDetails')"
                                        class="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-zinc-800 text-slate-600 dark:text-zinc-400 cursor-pointer">
                                        <Eye class="h-4 w-4" />
                                    </button>
                                    <button @click="downloadDocument(doc)" :title="t('adminDocuments.btnDownload')"
                                        class="p-1.5 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-950/50 text-blue-600 dark:text-blue-400 cursor-pointer">
                                        <Download class="h-4 w-4" />
                                    </button>
                                    <button @click="confirmDelete(doc)" :title="t('adminDocuments.btnDelete')"
            ...
            <div
                ...
                <div>{{ t('adminDocuments.totalRecords') }} <span class="font-bold text-slate-900 dark:text-white">{{
                    totalRecords }}</span></div>
                <div class="flex items-center gap-2">
                    <button :disabled="currentPage <= 1" @click="currentPage--; fetchDocuments();"
                    ...
                    <button :disabled="currentPage >= lastPage" @click="currentPage++; fetchDocuments();"
                        class="px-3 py-1.5 rounded-lg border border-slate-200 dark:border-zinc-800 disabled:opacity-40 cursor-pointer">
        ...
        <div v-if="showPreviewModal"
            ...
            <div
                class="bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-3xl w-full max-w-xl p-6 shadow-2xl space-y-4 relative">
                ...
                <div class="flex items-center gap-2 text-main-dark dark:text-main">
                    ...
                    <h3 class="text-base font-black text-slate-900 dark:text-white">{{
                        t('adminDocuments.columnFilename') }} #{{ selectedDoc?.id }}</h3>
                </div>
                ...
                <div v-else-if="selectedDoc" class="space-y-4 text-xs">
                    <div
                        class="space-y-2 p-4 rounded-2xl bg-slate-50 dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800">
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="font-bold text-slate-900 dark:text-white">{{ selectedDoc.document_type
                            }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-slate-400">Size:</span>
                            <span class="font-mono text-slate-700 dark:text-zinc-300">{{ selectedDoc.formatted_file_size
                            }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-slate-400">Owner:</span>
                            <span class="font-bold text-slate-900 dark:text-white">{{ selectedDoc.user?.name }} ({{
                                selectedDoc.user?.email }})</span>
                        </div>
                    </div>
                    ...
                    <div v-if="selectedDoc.recognized_data" class="space-y-2">
                        ...
                        <pre
                            class="p-3 rounded-xl bg-zinc-950 text-emerald-400 font-mono text-[11px] overflow-x-auto max-h-40">{{ JSON.stringify(selectedDoc.recognized_data, null, 2) }}</pre>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div v-if="showDeleteModal"
            ...
            <div
                ...
                <h3 class="text-base font-extrabold text-slate-900 dark:text-white">{{ t('adminDocuments.btnDelete') }}?
                </h3>
