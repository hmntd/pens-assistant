...
import type { AdminCalculationItem as CalculationItem, AdminCalculationDetail as CalculationDetail } from '@/types';
...
<template>
    <div class="space-y-6">
        ...
        <div
            class="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 bg-white dark:bg-zinc-900 p-4 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-xs">
            <div class="relative flex-1 max-w-md">
                <Search class="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 dark:text-zinc-500" />
                <input v-model="searchQuery" type="text" :placeholder="t('adminCalculations.searchPlaceholder')"
            ...
            <div class="flex items-center gap-2 flex-wrap sm:flex-nowrap text-xs">
                <div
                    class="flex items-center gap-1 bg-slate-50 dark:bg-zinc-950 px-3 py-1.5 rounded-xl border border-slate-200 dark:border-zinc-800">
        ...
        <div
                ...
                <table class="w-full text-left text-xs sm:text-sm min-w-[750px]">
                    <thead
                        ...
                        <tr>
                            ...
                            <th @click="handleSort('target_retirement_year')"
                                class="p-3.5 cursor-pointer hover:text-slate-900 dark:hover:text-white transition-colors">
                                <div class="flex items-center gap-1">
                                    <span>{{ t('adminUsers.retirementYearLabel') }}</span>
                                    <ChevronUp v-if="sortBy === 'target_retirement_year' && sortDir === 'asc'"
                                        class="h-3.5 w-3.5" />
                                    <ChevronDown v-if="sortBy === 'target_retirement_year' && sortDir === 'desc'"
                                        class="h-3.5 w-3.5" />
                                </div>
                            </th>
                            <th @click="handleSort('base_pension_amount')"
                                class="p-3.5 cursor-pointer hover:text-slate-900 dark:hover:text-white transition-colors">
                                <div class="flex items-center gap-1">
                                    <span>{{ t('adminCalculations.columnBasePension') }}</span>
                                    <ChevronUp v-if="sortBy === 'base_pension_amount' && sortDir === 'asc'"
                                        class="h-3.5 w-3.5" />
                                    <ChevronDown v-if="sortBy === 'base_pension_amount' && sortDir === 'desc'"
                                        class="h-3.5 w-3.5" />
                                </div>
                            </th>
                            <th @click="handleSort('final_pension_amount')"
                                class="p-3.5 cursor-pointer hover:text-slate-900 dark:hover:text-white transition-colors">
                                <div class="flex items-center gap-1">
                                    <span>{{ t('adminCalculations.columnFinalPension') }}</span>
                                    <ChevronUp v-if="sortBy === 'final_pension_amount' && sortDir === 'asc'"
                                        class="h-3.5 w-3.5" />
                                    <ChevronDown v-if="sortBy === 'final_pension_amount' && sortDir === 'desc'"
                                        class="h-3.5 w-3.5" />
                                </div>
                            </th>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-zinc-800/80 font-medium">
                        ...
                        <tr v-for="calc in calculations" :key="calc.id"
                            ...
                            <td class="p-3.5 font-bold text-slate-700 dark:text-zinc-300">{{ calc.target_retirement_year
                                }}</td>
                            <td class="p-3.5 font-mono text-slate-600 dark:text-zinc-300">{{
                                calc.base_pension_amount.toLocaleString('uk-UA', { minimumFractionDigits: 2 }) }} ₴</td>
                            <td
                                class="p-3.5 font-mono font-extrabold text-main-dark dark:text-main text-sm sm:text-base">
                            ...
                            <td class="p-3.5 text-right">
                                <div class="flex items-center justify-end gap-1.5">
                                    <button @click="viewCalculationDetails(calc)"
                                        :title="t('adminCalculations.btnViewBreakdown')"
                                        class="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-zinc-800 text-slate-600 dark:text-zinc-400 cursor-pointer">
                                        <Eye class="h-4 w-4" />
                                    </button>
                                    <button @click="confirmDelete(calc)" :title="t('adminCalculations.btnDelete')"
                                        class="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-950/50 text-red-600 dark:text-red-400 cursor-pointer">
            ...
            <div
                ...
                <div>{{ t('adminCalculations.totalRecords') }} <span class="font-bold text-slate-900 dark:text-white">{{
                    totalRecords }}</span></div>
                <div class="flex items-center gap-2">
                    <button :disabled="currentPage <= 1" @click="currentPage--; fetchCalculations();"
                    ...
                    <button :disabled="currentPage >= lastPage" @click="currentPage++; fetchCalculations();"
                        class="px-3 py-1.5 rounded-lg border border-slate-200 dark:border-zinc-800 disabled:opacity-40 cursor-pointer">
        ...
        <div v-if="showDetailModal"
            ...
            <div
                class="bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-3xl w-full max-w-2xl p-6 shadow-2xl space-y-6 relative max-h-[85vh] flex flex-col">
                ...
                <div v-else-if="selectedCalc"
                    class="flex-1 overflow-y-auto space-y-4 text-xs pr-1 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-slate-300 dark:[&::-webkit-scrollbar-thumb]:bg-zinc-800 [&::-webkit-scrollbar-thumb]:rounded-full">
                    <!-- Key Formula Indicators -->
                    <div
                        class="grid grid-cols-2 sm:grid-cols-4 gap-3 p-4 rounded-2xl bg-slate-50 dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800">
                        <div>
                            <span class="text-slate-400 block">Kz (Wage Coeff):</span>
                            <span class="text-base font-black text-slate-900 dark:text-white">{{
                                Number(selectedCalc.kz_wage_coefficient).toFixed(4) }}</span>
                        </div>
                        <div>
                            <span class="text-slate-400 block">Zp (Average):</span>
                            <span class="text-base font-black text-slate-900 dark:text-white">{{
                                Number(selectedCalc.zp_macroeconomic_average).toLocaleString('uk-UA') }} ₴</span>
                        </div>
                        <div>
                            <span class="text-slate-400 block">Ks (Service Coeff):</span>
                            <span class="text-base font-black text-slate-900 dark:text-white">{{
                                Number(selectedCalc.ks_service_coefficient).toFixed(4) }}</span>
                        </div>
                        <div>
                            <span class="text-slate-400 block">Final Pension:</span>
                            <span class="text-base font-black text-main-dark dark:text-main">{{
                                Number(selectedCalc.final_pension_amount).toLocaleString('uk-UA') }} ₴</span>
                        </div>
                    </div>
                ...
                <div v-if="selectedCalc"
                    class="pt-4 border-t border-slate-100 dark:border-zinc-800 flex items-center justify-end gap-2 shrink-0">
                    <a :href="`/admin/pension-calculations/${selectedCalc.id}/pdf?lang=${locale}`" download
        ...
        <div v-if="showDeleteModal"
            ...
            <div
                ...
                <h3 class="text-base font-extrabold text-slate-900 dark:text-white">{{ t('adminCalculations.btnDelete')
                    }}?</h3>
                ...
                <div class="flex items-center justify-end gap-2 pt-2">
                    ...
                    <button @click="executeDelete()" :disabled="isDeleting" type="button"
