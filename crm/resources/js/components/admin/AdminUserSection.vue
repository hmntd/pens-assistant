...
async function fetchUsers() {
    ...
    try {
        ...
        if (res.ok && data.status === 'success') {
            ...
            toast.error(data.message || t('adminUsers.errorFetch'));
        }
    } catch (err: any) {
        toast.error(t('adminUsers.networkErrorFetch'));
    } finally {
...
async function viewUserDetails(user: UserItem) {
    ...
    try {
        ...
        if (res.ok && data.status === 'success') {
            ...
            toast.error(t('adminUsers.errorDetailFetch'));
...
async function handleRoleChange(user: UserItem, newRole: string) {
    if (user.id === currentUserId.value) {
        toast.error(t('adminUsers.cannotBlockSelf'));
        fetchUsers();
        return;
    }
    try {
        ...
        if (res.ok && data.status === 'success') {
            ...
            toast.success(data.message || t('adminUsers.roleUpdated').replace(':role', newRole));
        } else {
            toast.error(data.message || t('adminUsers.errorRoleUpdate'));
            fetchUsers();
        }
    } catch (err: any) {
        toast.error(t('adminUsers.networkError'));
        fetchUsers();
    }
}
...
async function executeConfirmedAction() {
    ...
    try {
        ...
        if (res.ok && data.status === 'success') {
            ...
            toast.error(data.message || t('adminUsers.errorExecute'));
        }
    } catch (err: any) {
        toast.error(t('adminUsers.networkErrorExecute'));
    } finally {
...
<template>
    <div class="space-y-6">
        ...
        <div
            class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 bg-white dark:bg-zinc-900 p-4 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-xs">
            <div class="relative flex-1 max-w-md">
                <Search class="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 dark:text-zinc-500" />
                <input v-model="searchQuery" type="text" :placeholder="t('adminUsers.searchPlaceholder')"
        ...
        <div
            class="bg-white dark:bg-zinc-900 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-xs overflow-hidden">
            <div
                class="overflow-x-auto [&::-webkit-scrollbar]:h-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-slate-300 dark:[&::-webkit-scrollbar-thumb]:bg-zinc-800 [&::-webkit-scrollbar-thumb]:rounded-full">
                <table class="w-full text-left text-xs sm:text-sm min-w-[700px]">
                    <thead
                        class="bg-slate-50 dark:bg-zinc-950 text-slate-500 dark:text-zinc-400 font-extrabold uppercase tracking-wider text-[10px] sm:text-[11px] border-b border-slate-200 dark:border-zinc-800">
                        <tr>
                            <th @click="handleSort('id')"
                                class="p-3.5 cursor-pointer hover:text-slate-900 dark:hover:text-white transition-colors">
                            ...
                            <th @click="handleSort('created_at')"
                                ...
                                <div class="flex items-center gap-1">
                                    ...
                                    <ChevronUp v-if="sortBy === 'created_at' && sortDir === 'asc'"
                                        class="h-3.5 w-3.5" />
                                    <ChevronDown v-if="sortBy === 'created_at' && sortDir === 'desc'"
                                        class="h-3.5 w-3.5" />
                                </div>
                            </th>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-zinc-800/80 font-medium">
                        ...
                        <tr v-for="user in users" :key="user.id"
                            class="hover:bg-slate-50/80 dark:hover:bg-zinc-950/60 transition-colors"
                            :class="{ 'opacity-60 bg-red-50/20 dark:bg-red-950/10': user.is_trashed }">
                            <td class="p-3.5 font-bold font-mono text-slate-500">#{{ user.id }}</td>
                            <td class="p-3.5">
                                <div class="flex items-center gap-3">
                                    <!-- Avatar initials -->
                                    <div
                                        class="h-8 w-8 rounded-full bg-main/20 text-main-dark dark:text-main flex items-center justify-center font-bold text-xs shrink-0 border border-main/30">
                            ...
                            <td class="p-3.5">
                                <span v-if="user.is_trashed"
                                    class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold bg-red-100 text-red-700 dark:bg-red-950 dark:text-red-300">
                                    <XCircle class="h-3 w-3" /> {{ t('adminUsers.statusTrashed') }}
                                </span>
                                <span v-else-if="user.is_suspended"
                                    class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-300">
                                    <Ban class="h-3 w-3" /> {{ t('adminUsers.statusSuspended') }}
                                </span>
                                <span v-else
                                    class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300">
                            ...
                            <td class="p-3.5 text-right">
                                <div class="flex items-center justify-end gap-1.5">
                                    <!-- View Details -->
                                    <button @click="viewUserDetails(user)" :title="t('adminUsers.titleViewDetails')"
                                        class="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-zinc-800 text-slate-600 dark:text-zinc-400 cursor-pointer">
                                        <Eye class="h-4 w-4" />
                                    </button>

                                    <!-- Suspend / Ban Toggle -->
                                    <button @click="openConfirmModal(user, 'suspend')"
                                    ...
                                    <button v-if="user.is_trashed" @click="openConfirmModal(user, 'restore')"
                                        :title="t('adminUsers.titleRestore')"
                                        class="p-1.5 rounded-lg hover:bg-emerald-50 dark:hover:bg-emerald-950/50 text-emerald-600 dark:text-emerald-400 cursor-pointer">
                                        <RotateCcw class="h-4 w-4" />
                                    </button>
                                    <button v-else @click="openConfirmModal(user, 'delete')"
                                        :disabled="user.id === currentUserId"
            ...
            <!-- Pagination Bar -->
            <div
                class="flex items-center justify-between p-4 border-t border-slate-200 dark:border-zinc-800 text-xs text-slate-500">
                <div>{{ t('adminUsers.totalRecords') }} <span class="font-bold text-slate-900 dark:text-white">{{
                    totalRecords }}</span></div>
                <div class="flex items-center gap-2">
                    <button :disabled="currentPage <= 1" @click="currentPage--; fetchUsers();"
                    ...
                    <span>{{ t('adminUsers.pageWord') }} {{ currentPage }} {{ t('adminUsers.pageOf') }} {{ lastPage
                        }}</span>
                    <button :disabled="currentPage >= lastPage" @click="currentPage++; fetchUsers();"
                        class="px-3 py-1.5 rounded-lg border border-slate-200 dark:border-zinc-800 disabled:opacity-40 cursor-pointer">
        ...
        <div v-if="showDetailModal"
            ...
            <div
                class="bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-3xl w-full max-w-xl p-6 shadow-2xl space-y-6 relative max-h-[90vh] overflow-y-auto">
                <button @click="showDetailModal = false"
                    class="absolute right-5 top-5 text-slate-400 hover:text-slate-900 dark:hover:text-white cursor-pointer">
                    <X class="h-5 w-5" />
                </button>

                <div class="flex items-center gap-3">
                    <div
                        class="h-12 w-12 rounded-2xl bg-main/20 text-main-dark dark:text-main flex items-center justify-center font-bold text-lg border border-main/30">
                ...
                <div v-else-if="selectedUser" class="space-y-4 text-xs">
                    <div
                        class="grid grid-cols-2 gap-3 p-4 rounded-2xl bg-slate-50 dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800">
                        <div>
                            <span class="text-slate-400 block">{{ t('adminUsers.genderLabel') }}</span>
                            <span class="font-bold text-slate-900 dark:text-white">{{ selectedUser.gender === 'FEMALE' ?
                                t('adminUsers.genderFemale') : (selectedUser.gender === 'MALE' ?
                                    t('adminUsers.genderMale') : t('adminUsers.genderNotSpecified')) }}</span>
                        </div>
                        <div>
                            <span class="text-slate-400 block">{{ t('adminUsers.dobLabel') }}</span>
                            <span class="font-bold text-slate-900 dark:text-white">{{ selectedUser.date_of_birth ||
                                t('adminUsers.genderNotSpecified') }}</span>
                        </div>
                        <div>
                            <span class="text-slate-400 block">{{ t('adminUsers.disabilityGroupLabel') }}</span>
                            <span class="font-bold text-slate-900 dark:text-white">{{ selectedUser.disability_group ||
                                t('adminUsers.disabilityNone') }}</span>
                        </div>
                        <div>
                            <span class="text-slate-400 block">{{ t('adminUsers.retirementYearLabel') }}</span>
                            <span class="font-bold text-slate-900 dark:text-white">{{
                                selectedUser.target_retirement_year || t('adminUsers.retirementYearCurrent') }}</span>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        <div class="p-3 rounded-2xl bg-main/10 border border-main/20 text-center">
                            ...
                            <div class="text-base font-black text-slate-900 dark:text-white">{{
                                selectedUser.calculations_count || 0 }}</div>
                            <span class="text-[10px] text-slate-500">{{ t('adminUsers.statCalculations') }}</span>
                        </div>
                        <div
                            class="p-3 rounded-2xl bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-900 text-center">
                            <FileText class="h-5 w-5 text-blue-600 dark:text-blue-400 mx-auto mb-1" />
                            <div class="text-base font-black text-slate-900 dark:text-white">{{
                                selectedUser.documents_count || 0 }}</div>
                            <span class="text-[10px] text-slate-500">{{ t('adminUsers.statDocuments') }}</span>
                        </div>
                        <div
                            class="p-3 rounded-2xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-900 text-center">
                            <Calendar class="h-5 w-5 text-amber-600 dark:text-amber-400 mx-auto mb-1" />
                            <div class="text-base font-black text-slate-900 dark:text-white">{{
                                selectedUser.tax_histories_count || 0 }}</div>
        ...
        <div v-if="showConfirmModal"
            class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
            <div
                class="bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-3xl w-full max-w-md p-6 shadow-2xl space-y-4">
                <h3 class="text-base font-extrabold text-slate-900 dark:text-white">
                    <template v-if="confirmActionType === 'delete'">{{ t('adminUsers.modalDeleteTitle') }}</template>
                    <template v-else-if="confirmActionType === 'restore'">{{ t('adminUsers.modalRestoreTitle')
                        }}</template>
                    <template v-else-if="confirmActionType === 'suspend'">{{ t('adminUsers.modalSuspendTitle')
                        }}</template>
                </h3>
                <p class="text-xs text-slate-500 leading-relaxed">
                    {{ t('adminUsers.modalConfirmText') }} <span class="font-bold text-slate-900 dark:text-white">{{
                        targetUser?.name
                        }}</span>?
                </p>
                <div class="flex items-center justify-end gap-2 pt-2">
                    ...
                    <button @click="executeConfirmedAction()" :disabled="isProcessingAction" type="button"
