...
<template>
    <div class="space-y-6">
        ...
        <div class="space-y-4">
            ...
            <!-- Sessions List -->
            <div
                v-if="props.sessions && props.sessions.length > 0"
                class="divide-y divide-slate-100 overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm dark:divide-zinc-800/80 dark:border-zinc-800/80 dark:bg-zinc-950/50"
            >
                <div
                    v-for="session in props.sessions"
                    :key="session.id"
                    class="flex items-center justify-between p-4 transition-colors hover:bg-slate-50/50 sm:p-5 dark:hover:bg-zinc-900/50"
                >
                    <div class="flex items-center gap-4">
                        <div
                            class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-slate-700 dark:bg-zinc-900 dark:text-zinc-300"
                        >
                            <component
                        ...
                        <div class="space-y-1">
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                <span
                                    ...
                                    class="inline-flex items-center gap-1 rounded-full border border-emerald-500/20 bg-emerald-500/10 px-2.5 py-0.5 text-xs font-extrabold text-emerald-600 dark:text-emerald-400"
                                >
                    ...
                    >
                        <Button
                            variant="ghost"
                            size="sm"
                            class="text-slate-400 transition-colors hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-950/30 dark:hover:text-red-400"
                            :title="t('settings.sessions.revokeBtn')"
                            @click="revokeSession(session.id)"
                        >
                            <Trash2 class="h-4 w-4" />
                            <span class="sr-only">{{
                                t('settings.sessions.revokeBtn')
                            }}</span>
                        </Button>
                    </div>
                </div>
            ...
            <div
                v-else
                class="rounded-2xl border border-dashed border-slate-200 p-8 text-center dark:border-zinc-800"
            >
                <Globe
                    class="mx-auto h-8 w-8 text-slate-400 dark:text-zinc-600"
                />
            ...
            <div class="pt-2">
                <Button
                    variant="outline"
                    class="border-slate-300 font-bold text-slate-900 hover:bg-slate-100 dark:border-zinc-700 dark:text-white dark:hover:bg-zinc-800"
                    @click="confirmLogout"
                >
            ...
            <Dialog :open="confirmingLogout" @update:open="closeModal">
                <DialogContent class="sm:max-w-md">
                    <DialogHeader class="space-y-2">
                        ...
                        <DialogTitle class="text-center text-lg font-extrabold">
                            {{ t('settings.sessions.modalTitle') }}
                        </DialogTitle>
                        <DialogDescription
                            class="text-center text-xs text-slate-500 dark:text-zinc-400"
                        >
                    ...
                    >
                        <div class="space-y-2">
                            ...
                            <Input
                                ...
                                :placeholder="
                                    t('settings.sessions.passwordPlaceholder')
                                "
                        ...
                        <div class="flex items-center justify-end gap-3 pt-2">
                            ...
                            <Button
                                type="submit"
                                class="bg-main font-bold text-slate-950 hover:bg-main-hover"
