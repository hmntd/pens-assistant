    ...
    <section id="blog" class="py-16 lg:py-24">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col items-start justify-between gap-6 md:flex-row md:items-end">
                ...
                <div class="shrink-0">
                    <a :href="PFU_NEWS_PAGE_URL" target="_blank" rel="noopener noreferrer"
            ...
            <div class="mt-12 grid grid-cols-1 gap-8 md:grid-cols-3">
                <template v-if="news && news.length > 0">
                    <BlogCard v-for="(item, index) in news.slice(0, 3)" :key="item.id || index"
                        :tag="t('blog.pfuNewsTag')" :title="item.title" :date="item.published_at"
                        :description="item.preview_text || item.title" :url="item.url" />
                </template>
                <template v-else>
                    <BlogCard :tag="t('blog.pfuNewsTag')" :title="t('blog.post1Title')" :date="t('blog.post1Date')"
                        :description="t('blog.post1Desc')" :url="PFU_NEWS_PAGE_URL" />
                    <BlogCard :tag="t('blog.pfuNewsTag')" :title="t('blog.post2Title')" :date="t('blog.post2Date')"
                        :description="t('blog.post2Desc')" :url="PFU_NEWS_PAGE_URL" />
                    <BlogCard :tag="t('blog.pfuNewsTag')" :title="t('blog.post3Title')" :date="t('blog.post3Date')"
                        :description="t('blog.post3Desc')" :url="PFU_NEWS_PAGE_URL" />
                </template>
            </div>
        </div>
    </section>
