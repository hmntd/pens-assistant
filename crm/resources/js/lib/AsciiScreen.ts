...
export class AsciiScreen {
    ...
    _cleanupRenderer() {
        if (this._gl) {
            ...
            if (this._glFontTexture) {
                this._gl.deleteTexture(this._glFontTexture);
            }

            if (this._glVertexBuffer) {
                this._gl.deleteBuffer(this._glVertexBuffer);
            }

            if (this._glTexCoordBuffer) {
                this._gl.deleteBuffer(this._glTexCoordBuffer);
            }

            if (this._glColorBuffer) {
                this._gl.deleteBuffer(this._glColorBuffer);
            }
    ...
    _createFontTexture() {
        ...
        for (let t = 0; t < _.length; t++) {
            ...
            c.rect(e, s, r, l);
            c.clip();
            c.fillText(_[t], e + 0.5 * r, s + 0.5 * l);
            c.restore();
        }
        ...
        if (this._glFontTexture) {
            t.deleteTexture(this._glFontTexture);
        }
    ...
    _detectFullscreen(): boolean {
        ...
        while (t) {
            if (t.classList && t.classList.contains('is-fullscreen')) {
                return true;
            }
    ...
    _getCharMetrics() {
        ...
        const e = this._measureSpan
            ? this._measureSpan.getBoundingClientRect().width
            : 8;
    ...
    resize() {
        ...
        const l = Math.min(
            this.maxCols,
            Math.max(this.minCols, Math.floor(i / r)),
        );
        ...
        if (this.fullscreen) {
            ...
            if (t) {
                ...
                h = Math.min(
                    this.maxRows,
                    Math.max(this.minRows, Math.floor(l / o)),
                );
            } else {
                h = this.defaultRows;
            }
    ...
    put(t: number, e: number, s: string, i: number | string = 0) {
        if (t < 0 || t >= this.cols || e < 0 || e >= this.rows) {
            return;
        }
        ...
        if (this._forceFullRedraw) {
            ...
            for (let t = 0; t < h; t++) {
                ...
                const e =
                    'mono' === this.mode
                        ? '#ffffff'
                        : 'palette' === this.mode
                          ? this.palette[Number(r[t])] || this.palette[0]
                          : (r[t] as string) || '#ffffff';

                if (!this._colorBatches.has(e)) {
                    this._colorBatches.set(e, []);
                }
            ...
            for (const [s, r] of this._colorBatches) {
                ...
                for (const s of r) {
                    ...
                    t.fillText(i[s], r * l + 0.5 * l, h * o + 0.5 * o);
                }
            }
        } else {
            for (let s = 0; s < h; s++) {
                ...
                if (' ' !== h) {
                    t.fillStyle =
                        'mono' === this.mode
                            ? '#ffffff'
                            : 'palette' === this.mode
                              ? this.palette[Number(a)] || this.palette[0]
                              : (a as string) || '#ffffff';
                    t.fillText(h, f + 0.5 * l, _ + 0.5 * o);
                }
            }
        }
    }
    ...
    _renderWebGL() {
        ...
        for (let t = 0; t < n; t++) {
            ...
            if ('mono' === this.mode) {
                ...
                const e =
                    this.palette[Number(i[t])] || this.palette[0] || '#ffffff';
